...
{  Contributors: Ariel Montes, Lance Rasmussen, Claude Code                    }
{                                                                              }
...
procedure RegisterPanelThemeAttributes(const AVCLStyleName: string;
  const AThemeType: TThemeType;
  const AFontColor: TColor;
  const APanelColor: TColor;
  const ABorderColor: TColor;
...
function GetPanelStyleAttributes(const AStyleName: string;
  out AThemeAttribute: TPanelThemeAttribute): Boolean;
...
var
  ...
  PanelThemeAttributes: TList<TPanelThemeAttribute>;
...
function GetPanelStyleAttributes(const AStyleName: string;
  out AThemeAttribute: TPanelThemeAttribute): Boolean;
var
  LWindowsThemeAttr, LThemeAttribute: TPanelThemeAttribute;
begin
  LWindowsThemeAttr := nil;
  for LThemeAttribute in PanelThemeAttributes do
  begin
    if LThemeAttribute.StyleName = DEFAULT_WINDOWS_CLASS then
      LWindowsThemeAttr := LThemeAttribute;
    if SameText(AStyleName, LThemeAttribute.StyleName) then
    begin
      AThemeAttribute := LThemeAttribute;
      Exit(True); //Panel Theme Attributes found: Exit from function
    end;
  end;
  AThemeAttribute := LWindowsThemeAttr;
  if AThemeAttribute = nil then
  raise Exception.CreateFmt('Panel Attributes for Style "%s" not found!'+sLineBreak+
    'please call RegisterPanelThemeAttributes in an initialization section to add your custom style',[AStyleName])
  else
    Result := True;
end;
...
var
  ...
  begin
    ...
    LThemeAttribute.PanelColor     := APanelColor;
...
begin
  ...
  //not found
  LThemeAttribute := TPanelThemeAttribute.Create;
  PanelThemeAttributes.Add(LThemeAttribute);
  UpdateThemeAttributes;
end;
...
begin
  ...
  PanelThemeAttributes := TList<TPanelThemeAttribute>.Create;
  ...
  begin
    ...
    RegisterThemeAttributes('Calypso SE',ttDark,clWhite,clBlack,
      htmlToColor('#44617c'),htmlToColor('#44617c'),htmlToColor('#6bafe2'),
    ...
    RegisterThemeAttributes('Carbon',ttDark,htmlToColor('#c0c0c0'),clWhite,
      htmlToColor('#3a3a3a'),htmlToColor('#3a3a3a'),htmlToColor('#3e3e3e'),
    ...
    RegisterThemeAttributes('Charcoal Dark Slate',ttDark,htmlToColor('#a3a3a3'),clWhite,
      htmlToColor('#3a3a3a'),htmlToColor('#3a3a3a'),htmlToColor('#1f1f1f'),
    ...
    RegisterThemeAttributes('Cyan Dusk',ttLight,clBlack,clWhite,
      htmlToColor('#b4bac2'),htmlToColor('#b4bac2'),htmlToColor('#324a65'),
      htmlToColor('#687e97'),htmlToColor('#687e97'),btRoundRect);
    RegisterThemeAttributes('Cyan Night',ttLight,clBlack,clWhite,
      htmlToColor('#b0b4bf'),htmlToColor('#b0b4bf'),htmlToColor('#323c62'),
    ...
    RegisterThemeAttributes('Sterling',ttLight,clBlack,htmlToColor('#527593'),
      htmlToColor('#f9fafc'),htmlToColor('#f9fafc'),htmlToColor('#eef6fb'),
      htmlToColor('#dde1e4'),htmlToColor('#dde1e4'),btRoundRect);
    RegisterThemeAttributes('Tablet Dark',ttDark,clWhite,clBlack,
      htmlToColor('#3d4a79'),htmlToColor('#3d4a79'),htmlToColor('#1abc9c'),
    ...
    RegisterThemeAttributes('Tablet Light',ttLight,clBlack,clWhite,
      clWhite,clWhite,htmlToColor('#3d84dd'),
    ...
    RegisterThemeAttributes('Windows10 Blue Whale',ttDark,clWhite,clWhite,
      htmlToColor('#364762'),htmlToColor('#364762'),htmlToColor('#364762'),
    ...
    RegisterThemeAttributes('Windows11 MineShaft',ttDark,clWhite,clBlack,
      htmlToColor('#373737'),htmlToColor('#373737'),htmlToColor('#47b1e8'),
    ...
    RegisterThemeAttributes('Windows11 Impressive Dark',ttDark,clWhite,clWhite,
      htmlToColor('#3F506D'),htmlToColor('#3F506D'),htmlToColor('#5172EF'),
...
var
  LThemeAttribute: TThemeAttribute;
  LPanelThemeAttribute: TPanelThemeAttribute;
begin
  for LThemeAttribute in ThemeAttributes do
    LThemeAttribute.Free;
  ThemeAttributes.Free;

  for LPanelThemeAttribute in PanelThemeAttributes do
    LPanelThemeAttribute.Free;
  PanelThemeAttributes.Free;
end;
...
initialization
  ...
  InitDefaultPanelThemesAttributes;
