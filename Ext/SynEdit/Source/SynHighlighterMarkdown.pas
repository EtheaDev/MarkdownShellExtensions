...
uses
  ...
  System.Generics.Collections, System.SysUtils, SynEditHighlighter;

type
  TtkTokenKind = (tkUnknown, tkBlockQuote, tkCode, tkDelete, tkEmphasis,
    tkStrong, tkEntityReference, tkHeader, tkHtmlAttrName, tkHtmlAttrValue,
    tkHtmlComment, tkHtmlTag, tkLink, tkList, tkSpace);
  ...
  published
    ...
    property StrongAttri: TSynHighlighterAttributes read FStrongAttri
      write FStrongAttri;
...
begin
  ...
  FHtmlCommentAttri := TSynHighlighterAttributes.Create('HtmlComment',
    'HTML Comment');
  FHtmlCommentAttri.Foreground := clWebGreen;
  FHtmlCommentAttri.Style := [fsItalic];
  ...
  FListAttri.Foreground := clWebTeal;
...
begin
  ...
  if Ret.Success and (Run = (Ret.Index - 1)) then
  begin
    //Group 1 is the delimiter run: 1 = italic, 2/3 = bold
    if Ret.Groups[1].Length = 1 then
      FTokenID := tkEmphasis
    else
      FTokenID := tkStrong;
    Run := Run + Ret.Length;
...
var
  Ret: TMatch;
begin
  Ret := RegexEntityReference.Match(FLine, Run + 1);
  if Ret.Success and (Run = (Ret.Index - 1)) then
  begin
    if FLine[Run + 1] <> '#' then
    begin
